...
NT Domain : For active directory integration, the config nt_domain doesn't exist in hue.ini file. It has to be added to hue.ini. If your email identity is xyz@mycompany.com, then nt_domain gets the value "mycompany.com"
			While authenticationthenticating we will give our username('xyz') which will be framed as <username>@<nt_domain> (xyz@mycompany.com) and will be used as userPrincipalName (UPN). The password provided during login is used for binding.
...
bind_dn and bind_password : Needed for importing users in a group
